...
/*  Control characters are characters that do not represent printable symbols
   but instead serve to control or manipulate the appearance and behavior of
   the text. These characters are typically not visible when rendered or
   printed and are used for various purposes, such as formatting, signaling,
   and control. Few examples: Newline (Line Feed) (\n) Carriage Return     (\r)
            Tab                 (\t)
            Backspace           (\b)
            Form Feed           (\f)
            Vertical Tab        (\x0B)
